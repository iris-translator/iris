...
import { c } from 'd';
        ...
        let mut program =
            Parser::new(&allocator, code, SourceType::mjs()).with_options(options).parse().program;
        ...
        let transformed =
            transformer.transformer.build_with_symbols_and_scopes(symbols, scopes, &mut program);
